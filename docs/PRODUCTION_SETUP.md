...
- ⚙️ **GitHub Actions** → automatización cada 10 min
...
⚠️ **IMPORTANTE:** Verifica que veas la carpeta **`.github/`** (con el punto al inicio). Algunos sistemas la ocultan. En Windows: "Vista" → marcar "Elementos ocultos".
...
⚠️ **IMPORTANTE para usar GitHub Pages:**
...
⚠️ Reemplaza `TU_USUARIO` con tu usuario real de GitHub. Termina con `/`.
...
⚠️ **NOTA:** Los passwords mostrados son los **iniciales por defecto**. El sistema obliga al cambio en el primer login.
...
⚠️ El sistema te pedirá cambiar la password en tu primer login.
...
⚠️ **Importante:** Antes de enviar, abre el archivo y reemplaza `https://acm-dashboard.netlify.app` por tu URL real: `https://TU_USUARIO.github.io/acm-dashboard/`
